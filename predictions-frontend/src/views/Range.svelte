<script>
  ...
  // Calculate statistics for a row's predictions
  function calculateStats(row) {
    ...
    if (predictions.length === 0) {
      return { min: 0.5, max: 0.5, mean: 0.5, predictions: [] }
    }
    ...
    return { min, max, mean, predictions }
  }
...
<div class="flex flex-col gap-4">
  <h2 class="text-xl font-semibold text-text-primary">Prediction Range</h2>
        ...
        <table class="w-full text-sm">
          <thead>
            <tr class="bg-panel-mid border-b border-panel-border">
              ...
              <th class="px-2 md:px-3 py-2 text-center text-xs font-semibold uppercase tracking-wider text-text-secondary min-w-[220px]">Prediction Range</th>
            </tr>
          </thead>
          <tbody>
            {#each filteredData as row}
              ...
              >
                ...
                <td class="px-2 md:px-3 py-2">
                  <svg width="200" height="24" class="inline-block">
                    <!-- Prior line at 50% -->
                    <line x1="100" y1="0" x2="100" y2="24" stroke="#4a4a4f" stroke-dasharray="2,2"/>

                    <!-- Range bar -->
                    <rect
                      x={stats.min * 200}
                      y="8"
                      width={(stats.max - stats.min) * 200}
                      height="8"
                      fill="#4a4a4f"
                      rx="2"
                    />
                    ...
                    <!-- Player dots -->
                    {#each stats.predictions as p}
                      <circle cx={p.value * 200} cy="12" r="4" fill={p.color} opacity="0.8"/>
                    {/each}

                    <!-- Mean marker -->
                    <circle cx={stats.mean * 200} cy="12" r="5" fill="#fff" stroke="#333" stroke-width="1"/>

                    <!-- Outcome indicator -->
                    {#if row.outcome !== null}
                      <line
                        x1={row.outcome ? 200 : 0}
                        y1="2"
                        x2={row.outcome ? 200 : 0}
                        y2="22"
                        stroke={row.outcome ? '#4ade80' : '#f87171'}
                        stroke-width="3"
                      />
                    {/if}
                  </svg>
